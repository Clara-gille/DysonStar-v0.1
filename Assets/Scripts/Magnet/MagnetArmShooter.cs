...
namespace Magnet
{
    public class MagnetArmShooter : MonoBehaviour
    {
        ...
        /* Behaviour is :
        if only one button is pressed, the corresponding laser is shot
        if both buttons are pressed, the last one pressed is shot
        if both buttons were pressed and one is released, the other one is shot
     */
